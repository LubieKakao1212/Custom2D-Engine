...
namespace Custom2d_Engine.Scenes.Factory {
    public static class HierarchyUtils {
        ...
        public static T CreateChild<T>(this HierarchyObject thisObj, Func<T> constructor,
            Vector2 localPosition = default, float localRotation = 0f, Vector2? localScale = default)
        ...
        public static T CreateChild<T>(this HierarchyObject thisObj, Func<T> constructor, out T childOut,
            Vector2 localPosition = default, float localRotation = 0f, Vector2? localScale = default)
            where T : HierarchyObject {
        ...
        public static HierarchyObject CreateChild(this HierarchyObject thisObj, Vector2 localPosition = default,
            float localRotation = 0f, Vector2? localScale = default) {
            return thisObj.CreateChild(HierarchyFactory.HierarchyObjectFactory(), localPosition, localRotation,
                localScale);
        }

        public static DrawableObject CreateDrawableChild(this HierarchyObject thisObj, Sprite sprite,
            Vector2 localPosition = default, float localRotation = 0f, Vector2? localScale = default,
            Color? color = null, float drawOrder = 0f) {
            return thisObj.CreateChild(HierarchyFactory.DrawableObjectFactory(sprite, color, drawOrder), localPosition,
                localRotation, localScale);
        }

        public static OffsetObject CreateOffsetObject(this HierarchyObject thisObj, Vector2 offset = default,
            float rotation = 0f,
            Vector2? localScale = default, TransformSpace positionSpace = TransformSpace.Local,
            TransformSpace rotationSpace = TransformSpace.Local) {
