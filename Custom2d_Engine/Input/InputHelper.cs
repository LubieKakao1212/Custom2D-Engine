...
public static class InputHelper {
    ...
    /// <returns>returned binding must still be registered via <see cref="InputManager.RegisterBinding(IBindingInput)"/></returns>
    public static CompoundAxixBindingInput CreateSimpleAxisBinding(this InputManager manager, string name,
        Keys negative, Keys positive) {
    ...
    public static CompoundAxixBindingInput CreateSimpleKeysBinding(this InputManager manager, string name,
        params Keys[] bindings) {
        var binding = new CompoundAxixBindingInput(name);
    ...
    public static CompoundAxixBindingInput CreateSimpleBinding(this InputManager manager, string name,
        params ValueInputBase<bool>[] inputs) {
        var binding = new CompoundAxixBindingInput(name);
    ...
    public static ProcessorInput<float, float> Clamp(this ValueInputBase<float> input, float min = -1f,
        float max = 1f) {
    ...
    public static Vector2 DeadzoneHandler(Vector2 value, float deadzone) {
        var mag = Max(value.Length(), ContinuousInputBase<Vector2>.Epsilon);
        //TODO Normalize
