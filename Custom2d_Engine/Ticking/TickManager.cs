...
namespace Custom2d_Engine.Ticking {
    ...
    public class TickManager {
        ...
        public TickMachineBase AddSimpleRepeatingAction(object owner, Action action, float repeateInterval,
            float phase = 0f) {
            return AddSimpleRepeatingAction(owner, action, TimeSpan.FromSeconds(repeateInterval),
                TimeSpan.FromSeconds(phase));
        }

        public TickMachineBase AddSimpleRepeatingAction(object owner, Action action, TimeSpan repeateInterval,
            TimeSpan phase = default) {
            return AddTicker(owner, new SimpleRepeatingTickMachine(action, repeateInterval, phase));
        }

        public TickMachineBase AddAccurateRepeatingAction(object owner, Action action, float repeateInterval,
            float phase = 0f) {
            return AddAccurateRepeatingAction(owner, action, TimeSpan.FromSeconds(repeateInterval),
                TimeSpan.FromSeconds(phase));
        }

        public TickMachineBase AddAccurateRepeatingAction(object owner, Action action, TimeSpan repeateInterval,
            TimeSpan phase = default) {
            return AddTicker(owner, new AccurateRepeatingTickMachine(action, repeateInterval, phase));
        }

        public TickMachineBase AddActionSequence(object owner, IEnumerator<TimeSpan> sequence,
            TimeSpan phase = default) {
            return AddTicker(owner, new SequenceTickMachine(sequence, TimeSpan.Zero, phase));
        }

        public TickMachineBase AddRepeatingActionSequence(object owner, IEnumerable<TimeSpan> sequence,
            TimeSpan phase = default) {
            throw new NotImplementedException("This is on TODO list");
        }

        public void RemoveTicker(object owner, ITickMachine ticker) {
            _actions.GetValueOrDefault(owner)?.Remove(ticker);
        }
